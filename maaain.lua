...
local AccentColor = Color3.fromRGB(150, 0, 255)
local MainColor = Color3.fromRGB(10, 0, 15)
local SecondaryColor = Color3.fromRGB(30, 5, 35)
local BorderColor = Color3.fromRGB(100, 40, 120)
local ContentColor = Color3.fromRGB(180, 100, 200)
...
WatermarkStroke.Transparency = 0
...
local pulseItem, pulseText = CreateWatermarkItem("link-2", "| pulse.win", Color3.fromRGB(255, 255, 255))
...
Logo.Image = "rbxassetid://119364620638980"
...
local pulseWinLabel = Instance.new("TextLabel")
pulseWinLabel.Parent = Header
pulseWinLabel.Size = UDim2.new(0, 50, 0, 22)
pulseWinLabel.Position = UDim2.new(0, 105, 0, 5)
pulseWinLabel.BackgroundTransparency = 1
pulseWinLabel.Text = ".win"
pulseWinLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
pulseWinLabel.TextSize = 20
pulseWinLabel.Font = Enum.Font.GothamBold
pulseWinLabel.TextXAlignment = Enum.TextXAlignment.Left
...
RightPanel.BackgroundColor3 = Color3.fromRGB(15, 0, 20)
...
PulseFrame.Parent = RightHeader
PulseFrame.Size = UDim2.new(0, 120, 0, 40)
PulseFrame.Position = UDim2.new(0.22, 0, 0.5, 0)
PulseFrame.AnchorPoint = Vector2.new(1, 0.5)
PulseFrame.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
PulseFrame.BackgroundTransparency = 0.35
PulseFrame.ClipsDescendants = true

local PulseCorner = Instance.new("UICorner")
PulseCorner.Parent = PulseFrame
PulseCorner.CornerRadius = UDim.new(0, 3)

local PulseStroke = Instance.new("UIStroke")
PulseStroke.Parent = PulseFrame
PulseStroke.Transparency = 0.65
local pulseStrokeGrad = Instance.new("UIGradient")
pulseStrokeGrad.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(150, 0, 255)),
    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(0, 100, 255)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(150, 0, 255))
})
pulseStrokeGrad.Rotation = 90
pulseStrokeGrad.Parent = PulseStroke
PulseStroke.Color = Color3.fromRGB(255, 255, 255)

local SHAPE = {
    Vector2.new(0.00, 0.50), Vector2.new(0.10, 0.50), Vector2.new(0.20, 0.50),
    Vector2.new(0.30, 0.50), Vector2.new(0.37, 0.50), Vector2.new(0.42, 0.15),
    Vector2.new(0.45, 0.50), Vector2.new(0.48, 0.90), Vector2.new(0.51, 0.50),
    Vector2.new(0.57, 0.50), Vector2.new(0.63, 0.50), Vector2.new(0.70, 0.50),
    Vector2.new(0.78, 0.50), Vector2.new(0.86, 0.50), Vector2.new(0.95, 0.50),
...
for i = 1, numPoints do
    ...
    point.Size = UDim2.new(0, 2, 0, 0)
    point.Position = UDim2.new(0, 5 + (i-1) * spacing, 0.5, 0)
    point.AnchorPoint = Vector2.new(0.5, 0.5)
    local colorIndex = (i % 3) + 1
    point.BackgroundColor3 = pulseColors[colorIndex]
    point.BackgroundTransparency = 1
    local corner = Instance.new("UICorner")
    corner.Parent = point
    corner.CornerRadius = UDim.new(1, 0)
    table.insert(pulsePoints, point)
end
...
local function AnimatePulse()
    while PulseFrame and PulseFrame.Parent do
        ...
        local headX = progress <= 0.5 and progress * 2 or 2 - progress * 2
        for i, point in ipairs(pulsePoints) do
            local x = i / #pulsePoints
            local y = GetY(x)
            local dist = math.abs(x - headX)
            if dist < TRAIL_FRACTION then
                local alpha = 1 - dist / TRAIL_FRACTION
                point.BackgroundTransparency = 1 - alpha
                point.Size = UDim2.new(0, 2, 0, math.abs(y - 0.5) * 30 + 2)
                point.Position = UDim2.new(0, 5 + (i-1) * spacing, 0.5, -(y - 0.5) * 30)
            else
                point.BackgroundTransparency = 1
            end
        end
...
local Config = {
    ...
    ShaderRTX = false, BloomIntensity = 0.3, BloomSize = 10, BlurSize = 5,
    ShadersBrightness = 0.1, ShadersContrast = 0.5, ShadersSaturation = -0.3,
    SunRaysIntensity = 0.075, ShadersTime = 14, ShadersFog = 0.364, ShadersVignette = 0.3,
    FullBrightEnabled = false, NoFogEnabled = false, NoRenderEnabled = false, NoRenderColor = Color3.fromRGB(0, 0, 0),
    ...
    UltimateSkybox = "default", UltimateShadersOn = false,
...
local RTXSection = VisualsTab:AddSection({Name = "RTX SHADERS", Position = "left", Icon = GetIcon("camera")})
RTXSection:AddToggle({Name = "RTX Shaders", Icon = GetIcon("film"), Default = false, Callback = function(v) Config.ShaderRTX = v end, Flag = "ShaderRTX"})
RTXSection:AddSlider({Name = "Bloom Intensity", Icon = GetIcon("sun"), Default = 0.3, Min = 0, Max = 1, Rounding = 2, Type = "", Callback = function(v) Config.BloomIntensity = v end, Flag = "BloomIntensity"})
RTXSection:AddSlider({Name = "Bloom Size", Icon = GetIcon("maximize"), Default = 10, Min = 0, Max = 20, Type = "", Callback = function(v) Config.BloomSize = v end, Flag = "BloomSize"})
RTXSection:AddSlider({Name = "Blur Size", Icon = GetIcon("eye-off"), Default = 5, Min = 0, Max = 15, Type = "", Callback = function(v) Config.BlurSize = v end, Flag = "BlurSize"})
RTXSection:AddSlider({Name = "Brightness", Icon = GetIcon("sun"), Default = 0.1, Min = -0.5, Max = 0.5, Rounding = 2, Type = "", Callback = function(v) Config.ShadersBrightness = v end, Flag = "ShadersBrightness"})
RTXSection:AddSlider({Name = "Contrast", Icon = GetIcon("contrast"), Default = 0.5, Min = -0.5, Max = 1, Rounding = 2, Type = "", Callback = function(v) Config.ShadersContrast = v end, Flag = "ShadersContrast"})
RTXSection:AddSlider({Name = "Saturation", Icon = GetIcon("droplet"), Default = -0.3, Min = -1, Max = 1, Rounding = 2, Type = "", Callback = function(v) Config.ShadersSaturation = v end, Flag = "ShadersSaturation"})
RTXSection:AddSlider({Name = "SunRays Intensity", Icon = GetIcon("sun"), Default = 0.075, Min = 0, Max = 0.5, Rounding = 3, Type = "", Callback = function(v) Config.SunRaysIntensity = v end, Flag = "SunRaysIntensity"})
RTXSection:AddSlider({Name = "Day / Night (0-24h)", Icon = GetIcon("clock"), Default = 14, Min = 0, Max = 24, Type = "h", Callback = function(v) Config.ShadersTime = v end, Flag = "ShadersTime"})
RTXSection:AddSlider({Name = "Fog Density", Icon = GetIcon("cloud"), Default = 0.364, Min = 0, Max = 1, Rounding = 3, Type = "", Callback = function(v) Config.ShadersFog = v end, Flag = "ShadersFog"})
RTXSection:AddSlider({Name = "Vignette Opacity", Icon = GetIcon("circle"), Default = 0.3, Min = 0, Max = 1, Rounding = 2, Type = "", Callback = function(v) Config.ShadersVignette = v end, Flag = "ShadersVignette"})
...
local CloudsSection = VisualsTab:AddSection({Name = "CLOUDS", Position = "right", Icon = GetIcon("cloud")})
CloudsSection:AddSlider({Name = "Cloud Cover", Icon = GetIcon("cloud"), Default = 0, Min = 0, Max = 1, Rounding = 2, Type = "", Callback = function(v) Config.UltimateCloudCover = v end, Flag = "UltimateCloudCover"})
CloudsSection:AddSlider({Name = "Cloud Density", Icon = GetIcon("cloud"), Default = 0, Min = 0, Max = 1, Rounding = 2, Type = "", Callback = function(v) Config.UltimateCloudDensity = v end, Flag = "UltimateCloudDensity"})

local WaterSection = VisualsTab:AddSection({Name = "WATER", Position = "right", Icon = GetIcon("droplet")})
WaterSection:AddSlider({Name = "Water Speed", Icon = GetIcon("zap"), Default = 10, Min = 0, Max = 100, Type = "", Callback = function(v) Config.UltimateWaterSpeed = v end, Flag = "UltimateWaterSpeed"})
WaterSection:AddSlider({Name = "Water Transparency", Icon = GetIcon("eye"), Default = 0.3, Min = 0, Max = 1, Rounding = 2, Type = "", Callback = function(v) Config.UltimateWaterTrans = v end, Flag = "UltimateWaterTrans"})
WaterSection:AddSlider({Name = "Water Wave Size", Icon = GetIcon("maximize"), Default = 5, Min = 0, Max = 40, Type = "", Callback = function(v) Config.UltimateWaterSize = v end, Flag = "UltimateWaterSize"})
...
local DOFSection = VisualsTab:AddSection({Name = "DEPTH OF FIELD", Position = "left", Icon = GetIcon("focus")})
DOFSection:AddToggle({Name = "DOF Enabled", Icon = GetIcon("focus"), Default = false, Callback = function(v) Config.UltimateDOFEnabled = v end, Flag = "UltimateDOFEnabled"})
DOFSection:AddSlider({Name = "Far Intensity", Icon = GetIcon("arrow-right"), Default = 0.5, Min = 0, Max = 1, Rounding = 2, Type = "", Callback = function(v) Config.UltimateDOFFarIntensity = v end, Flag = "UltimateDOFFarIntensity"})
DOFSection:AddSlider({Name = "Focus Distance", Icon = GetIcon("target"), Default = 50, Min = 0, Max = 200, Type = "", Callback = function(v) Config.UltimateDOFFocusDist = v end, Flag = "UltimateDOFFocusDist"})
DOFSection:AddSlider({Name = "In Focus Radius", Icon = GetIcon("circle"), Default = 5, Min = 0, Max = 50, Type = "", Callback = function(v) Config.UltimateDOFInFocus = v end, Flag = "UltimateDOFInFocus"})
DOFSection:AddSlider({Name = "Near Intensity", Icon = GetIcon("arrow-left"), Default = 0.3, Min = 0, Max = 1, Rounding = 2, Type = "", Callback = function(v) Config.UltimateDOFNearIntensity = v end, Flag = "UltimateDOFNearIntensity"})

local SunRaysSection = VisualsTab:AddSection({Name = "SUN RAYS", Position = "left", Icon = GetIcon("sun")})
SunRaysSection:AddToggle({Name = "Sun Rays Enabled", Icon = GetIcon("sun"), Default = false, Callback = function(v) Config.UltimateSunRaysEnabled = v end, Flag = "UltimateSunRaysEnabled"})
SunRaysSection:AddSlider({Name = "Sun Rays Intensity", Icon = GetIcon("sun"), Default = 0.075, Min = 0, Max = 1, Rounding = 3, Type = "", Callback = function(v) Config.UltimateSunRaysIntensity = v end, Flag = "UltimateSunRaysIntensity"})
SunRaysSection:AddSlider({Name = "Sun Rays Spread", Icon = GetIcon("maximize"), Default = 0.727, Min = 0, Max = 1, Rounding = 3, Type = "", Callback = function(v) Config.UltimateSunRaysSpread = v end, Flag = "UltimateSunRaysSpread"})

local ColorCorSection = VisualsTab:AddSection({Name = "COLOR CORRECTION", Position = "right", Icon = GetIcon("droplet")})
ColorCorSection:AddToggle({Name = "Color Correction", Icon = GetIcon("droplet"), Default = true, Callback = function(v) Config.UltimateColorCorEnabled = v end, Flag = "UltimateColorCorEnabled"})
ColorCorSection:AddSlider({Name = "Brightness", Icon = GetIcon("sun"), Default = 0.1, Min = -1, Max = 1, Rounding = 2, Type = "", Callback = function(v) Config.UltimateColorBrightness = v end, Flag = "UltimateColorBrightness"})
ColorCorSection:AddSlider({Name = "Contrast", Icon = GetIcon("contrast"), Default = 0.5, Min = -1, Max = 1, Rounding = 2, Type = "", Callback = function(v) Config.UltimateColorContrast = v end, Flag = "UltimateColorContrast"})
ColorCorSection:AddSlider({Name = "Saturation", Icon = GetIcon("droplet"), Default = -0.3, Min = -1, Max = 1, Rounding = 2, Type = "", Callback = function(v) Config.UltimateColorSaturation = v end, Flag = "UltimateColorSaturation"})

local UltimateBloomSection = VisualsTab:AddSection({Name = "BLOOM (Ultimate)", Position = "right", Icon = GetIcon("sun")})
UltimateBloomSection:AddToggle({Name = "Bloom Enabled", Icon = GetIcon("sun"), Default = true, Callback = function(v) Config.UltimateBloomEnabled = v end, Flag = "UltimateBloomEnabled"})
UltimateBloomSection:AddSlider({Name = "Bloom Intensity", Icon = GetIcon("sun"), Default = 0.3, Min = 0, Max = 1, Rounding = 2, Type = "", Callback = function(v) Config.UltimateBloomIntensity = v end, Flag = "UltimateBloomIntensity"})
UltimateBloomSection:AddSlider({Name = "Bloom Size", Icon = GetIcon("maximize"), Default = 10, Min = 0, Max = 56, Type = "", Callback = function(v) Config.UltimateBloomSize = v end, Flag = "UltimateBloomSize"})
UltimateBloomSection:AddSlider({Name = "Bloom Threshold", Icon = GetIcon("target"), Default = 0.8, Min = 0, Max = 4, Rounding = 2, Type = "", Callback = function(v) Config.UltimateBloomThreshold = v end, Flag = "UltimateBloomThreshold"})

local UltimateBlurSection = VisualsTab:AddSection({Name = "BLUR (Ultimate)", Position = "right", Icon = GetIcon("eye-off")})
UltimateBlurSection:AddToggle({Name = "Blur Enabled", Icon = GetIcon("eye-off"), Default = false, Callback = function(v) Config.UltimateBlurEnabled = v end, Flag = "UltimateBlurEnabled"})
UltimateBlurSection:AddSlider({Name = "Blur Size", Icon = GetIcon("eye-off"), Default = 5, Min = 0, Max = 56, Type = "", Callback = function(v) Config.UltimateBlurSize = v end, Flag = "UltimateBlurSize"})
...
MiscVisualsSection:AddToggle({Name = "No Fog", Icon = GetIcon("eye"), Default = false, Callback = function(state)
...
end, Flag = "NoFogEnabled"})
MiscVisualsSection:AddToggle({Name = "No Render", Icon = GetIcon("eye-off"), Default = false, Callback = function(state)
...
end, Flag = "NoRenderEnabled"})

local MagicAuraSection = VisualsTab:AddSection({Name = "MAGIC AURA", Position = "right", Icon = GetIcon("sparkles")})
...
local ConfigSection = ConfigTab:AddSection({Name = "CONFIG", Position = "left", Icon = GetIcon("settings")})
local configLabel = ConfigSection:AddLabel({Name = "Current Config: Default", Icon = GetIcon("file-text")})
local configInput = ConfigSection:AddTextInput({Name = "Config Name", Icon = GetIcon("edit-3"), Placeholder = "Enter name...", Default = "", Size = 150})
...
local function SaveConfig(name)
    ...
    if writefile then
        writefile("Pulse/" .. name .. ".json", json)
    end
...
local function LoadConfig(name)
    if not name or name == "" then return end
    if readfile then
        local json = readfile("Pulse/" .. name .. ".json")
        if json then
...
ConfigSection:AddButton({Name = "Save Config", Icon = GetIcon("save"), Callback = function() SaveConfig(configInput:GetValue()) end})
ConfigSection:AddButton({Name = "Load Config", Icon = GetIcon("folder-open"), Callback = function() LoadConfig(configInput:GetValue()) end})
ConfigSection:AddButton({Name = "Delete Config", Icon = GetIcon("trash-2"), Callback = function() DeleteConfig(configInput:GetValue()) end})

task.delay(1, function()
    if readfile and isfile then
        if isfile("Pulse/Default.json") then
            LoadConfig("Default")
        else
            SaveConfig("Default")
        end
...
elseif mode == "Slide" then
    -- Мгновенное переключение между +range и -range
    local sign = math.sign(math.sin(spinAngle * 0.05))
    yAngle = math.rad(sign * (Config.SlideRange or 15))
